...
contract CrowdfundingProject is AutomationCompatibleInterface, AutomationBase {
    ...
    error CrowdfundingProject__AmountEthSentIsGreaterThanTheRestCrowfundingNeeded(uint256);
    error CrowdfundingProject__FundingIsNotActive();
    error CrowdfundingProject__CanOnlyBeCalledByTheCrowdfundingContract();
    error CrowdfundingProject__ContractHasLessEthThenNeededToPayOutAllInvestors(uint256);
    ...
    uint256 private immutable i_crowfundingAmountNeeded;
    ...
    constructor(
        ...
        i_crowfundingAmountNeeded = _maxCrowfundingAmount;
        s_fundedAmount = 0;
    ...
    function fund(address _investor) external payable onlyCrowdfundingContract {
        ...
        uint256 restFundingNeeded = i_crowfundingAmountNeeded - s_fundedAmount;
        ...
        s_investors.push() = Investor(payable(_investor), msg.value, amountToBePaid, false);
    ...
    function calculateAmountToBePaidToInvestor(
        ...
        return _amountInvested + ((_amountInvested * _interestRateInPercent) / 100);
    }
    ...
    function performUpkeep(bytes calldata /* performData */) external override {
        ...
        if (
            ...
            if (s_fundedAmount < i_crowfundingAmountNeeded) {
    ...
    function payBackInvestorsAndOwner() internal {
        ...
        for (uint256 i = 0; i < s_investors.length; i++) {
            uint256 amountInvested = s_investors[i].amountInvested;
            (bool success, ) = (s_investors[i].investor).call{value: amountInvested}("");
    ...
    function withdrawFunds() internal {
        ...
        (bool success2, ) = (s_crowdfundingContractAddress).call{value: address(this).balance}("");
    ...
    function finish() external onlyCrowdfundingContract {
        ...
        for (uint256 i = 0; i < s_investors.length; i++) {
            if (s_investors[i].paidOut == false) {
                uint256 amountToPayOut = s_investors[i].amountToBePaidOut;
                s_investors[i].paidOut = true;
                s_investors[i].amountToBePaidOut = 0;
                (bool success, ) = (s_investors[i].investor).call{value: amountToPayOut}("");
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < s_investors.length; i++) {
            if (s_investors[i].paidOut == false) {
                fullAmountToBePaid += s_investors[i].amountToBePaidOut;
            }
        }
    ...
    function getProjectMaxCrowdfundingAmount() external view returns (uint256) {
        return i_crowfundingAmountNeeded;
    }
    ...
    function getInvestorsInvestmentAmount(uint256 _index) external view returns (uint256) {
        return s_investors[_index].amountInvested;
    }

    function getInvestorsAmountToBePaidOut(uint256 _index) external view returns (uint256) {
        return s_investors[_index].amountToBePaidOut;
    }

    function getInvestorsPaidOutStatus(uint256 _index) external view returns (bool) {
